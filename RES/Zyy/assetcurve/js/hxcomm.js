 ...
 * return		: 股东账号
 * *****************************************************************/
function fnGetGDZH(jsonParam)
{
	if(IsClientVersionSupport('getGdzhByUserkey',jsonParam["xd_version"]))
	{
		var objParam = {};
		objParam["userkey"] = jsonParam["userkey"];
		objParam["setcode"] = jsonParam["set_code"];
		var strParam = window.JSON2.stringify(objParam);
		
		return fnFunction("get_gdzh", strParam);
	}
	else
	{
		return fnGetGDZHBySetCode(jsonParam["set_code"]);
	}
}
...
//客户端置顶函数
function fnSetPageOnTop()
{
	return fnFunction('set_window_hwndtop', '1');
}

//客户端取消置顶函数
function fnCancelPageOnTop()
{
	return fnFunction('set_window_hwndtop', '0');
}
...
//判断账户有无登录客户端 1代表已登录，0代表未登录
function fnIsAlreadyLogin(nAccountNum)
{
	if('0' == fnFunction('is_zjzh_logined',nAccountNum))
		return false;
	else
		return true;
}

//判断账户是否被锁定 1,代表已锁定 0代表未锁定
function fnIsLockLogin()
{
	if('1' == fnFunction('get_lock_status'))
		return true;
	else
		return false;
}

//获取下单启动模式  10 快捷方式启动 23 独立启动 其他为行情下启动
function fnGetLaunchMode()
{
	var xdType = fnFunction("get_hq_callway");
	if(xdType == '23' || xdType == '10')
	{
		return true;
	}
	else
...
//通过市场名称获取行情市场代码
function fnGetMarketIdByScdm(strJsonParam)
{
	return (fnFunction("get_hq_codetype",strJsonParam) || '');
}
//获取下单模式 精简 or 还原
function fnGetSimpleMode()
{
	return fnFunction("is_simple_mode");
}
...
/**
 * @description 写本地配置
 * @param {string} strSection 配置section
 * @param {strKey} strKey 配置key
 * @param {string} strValue 配置value
 * @param {string} strPath 配置文件的路径
 */
 function writeConfig(strSection, strKey, strValue, strPath) {
	 var objWriteData = { //nosonar
		 'section'	: strSection,
		 'key'		: strKey,
		 'value'	: strValue,
		 'file_path': strPath
	 };
	 var strParam = window.JSON2.stringify(objWriteData);
	 fnFunction('write_cfg', strParam);
 }
 
 /**
  * @description 读取本地配置
  * @param {*} strSection 配置section
  * @param {*} strKey 配置key
  * @param {*} strDefault 配置默认值
  * @param {*} strPath 配置文件路径
  * @returns 
  */
 function readConfig(strSection, strKey, strDefault, strPath)
 {
	 var objReadData = { //nosonar
		 'section': strSection,
		 'key': strKey,
		 'default': strDefault,
		 'file_path': strPath
	 };	
	 var param = window.JSON2.stringify(objReadData); //nosonar
	 var strConfig = fnFunction('read_cfg', param); //nosonar
